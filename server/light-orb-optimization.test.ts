...
describe("Proactive Intervention Rules", () => {
  ...
  it("triggers guided exploration at 90s idle", () => {
    ...
    const matchingRules = PROACTIVE_RULES.filter(r => r.condition(metrics, "creative"));
    const explorationRule = matchingRules.find(r => r.message("creative").includes("其他人的作品"));
...
function shouldTriggerInvitation(intent: IntentResult | null): boolean {
  ...
  const highTriggerTypes = ["choice_paralysis", "inspiration_seeking", "passive_browsing"];
...
describe("Invitation Trigger Logic (Enhanced)", () => {
  ...
  it("returns false for very low confidence (<0.3)", () => {
    expect(shouldTriggerInvitation(makeIntent({ confidence: 0.2 }))).toBe(false);
  });

  it("triggers for choice_paralysis at confidence 0.3", () => {
    expect(shouldTriggerInvitation(makeIntent({
      intentType: "choice_paralysis",
      confidence: 0.35,
    }))).toBe(true);
  });
...
describe("Welcome Message Logic", () => {
  function getGreeting(visitCount: number, hoursSinceLastVisit: number, hour: number): string | null {
    if (visitCount === 1) {
      return hour < 12
