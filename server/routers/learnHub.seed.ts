...
// 純資料 + 型別，無任何邏輯/副作用。不得 import learnHub.ts（避免循環依賴）。
...
export const SEED_DOCS: LearnDoc[] = [
  // ══════════════════════════════════════════════════════
  // 入門指南
  // ══════════════════════════════════════════════════════
...
`,
  ...
  // ══════════════════════════════════════════════════════
  // 核心人格 0→1 上手教材（AIDV-813 電商/自媒體/教育者 + AIDV-966 接案者/內容編輯/品牌方）
  // id 對齊 client/src/shells/learn/learnContent.ts 的 METHODOLOGY_DOCS（scenario-*），
  // 讓 /learn?docId=scenario-* deep-link 經 learnHub.getById 可解析（純內容註冊，無邏輯）。
  // ══════════════════════════════════════════════════════

  {
    id: "scenario-ecommerce-image",
    category: "getting-started",
    title: "電商賣家 × 圖：3 步把商品照變主圖",
    summary: "上手路徑：傳商品照→去背→換情境背景→多張主圖。常見卡關：不知去背/重繪背景；多張圖商品不一致；不知怎麼描述「白底主圖」。推薦功能：ImageStudio 去背＋圖生圖、一致性保險庫（Vault）、資產庫批次匯出。",
    content: `# 電商賣家 × 圖：3 步把商品照變主圖

## 🛒 這份教材給誰
你有商品實拍照，想快速做出平台規格的白底主圖與情境圖，不想學修圖軟體。

## 🚀 上手路徑（3 步）

### 第 1 步：傳商品照＋去背
到圖片創作室（/image-studio）上傳商品實拍照，用圖片編輯模型去背。
> 你會學到：去背不是修圖師專利，一句「remove background, keep product edges clean」就能做到。

### 第 2 步：換情境背景
用圖生圖把去背後的商品放進情境：白底主圖、生活情境、節慶氛圍。
> 你會學到：描述「白底主圖」的關鍵字：pure white background, soft studio lighting, subtle shadow, centered product。

### 第 3 步：多張主圖保持商品一致
把最滿意的一張存入一致性保險庫（/vault），之後每張情境圖都引用它，商品不會張張走樣。
> 你會學到：多張圖的一致性靠「引用同一張定版圖」，不是靠重複描述。

## 🧯 常見卡關與自救
- **不知道能去背／重繪背景** → 圖片創作室的「圖片編輯」分類就有；先去背再換景，比直接重生成穩。
- **多張圖商品長得不一樣** → 把定版商品圖存入保險庫，每次生成都引用同一張。
- **不知怎麼描述白底主圖** → 直接用提示詞庫的「電商商品白底圖」範本，改成你的商品名。

## 🔗 推薦功能
- 圖片創作室去背＋圖生圖（/image-studio）
- 一致性保險庫（/vault）
- 資產庫（/assets）：定稿歸檔與下載
`,
    tags: ["電商", "商品圖", "去背", "白底主圖", "入門"],
    difficulty: "beginner",
    readingMinutes: 8,
  ...
  {
    id: "scenario-creator-video",
    category: "getting-started",
    title: "自媒體 × 影：從一張圖到 15 秒短影音",
    summary: "上手路徑：縮圖或一句腳本→圖生影→短影音→配字幕／音樂。常見卡關：影片在背景跑但不知道；模型選擇困難；不知先 480p 試。推薦功能：VideoStudio（Kling v2.1、先 480p）、背景任務中心、導演 AI。",
    content: `# 自媒體 × 影：從一張圖到 15 秒短影音

## 📱 這份教材給誰
你經營社群帳號，想把一張圖或一句腳本變成能發的短影音，預算和時間都有限。
...
### 第 2 步：圖生影（先 480p 試水溫）
到影片工作室（/video-studio）選 Kling v2.1，寫一句鏡頭描述（如「鏡頭緩緩推近」），解析度先選 480p。
> 你會學到：先用 480p 便宜快速驗證運鏡，滿意再升解析度，省點數也省時間。

### 第 3 步：背景任務中心看進度
影片生成需要幾分鐘，任務會在背景跑；到背景任務中心（/background-tasks）就能看到進度，不用守著頁面。
> 你會學到：生成中可以繼續做別的事，完成會有通知。

### 第 4 步：配字幕／音樂
到音樂配音創作室（/pro-studio）加旁白（TTS）或背景音樂，完成 15 秒成片。
> 你會學到：一句話生成旁白與純音樂，不需要素材庫。

## 🧯 常見卡關與自救
- **按了生成沒反應？** → 影片在背景跑；到背景任務中心看進度。
- **模型太多不會選** → 短影音入門選 Kling v2.1；要更高品質再試 Veo。
- **一次生成花太多點** → 永遠先 480p 試運鏡，定稿才上高解析度。

## 🔗 推薦功能
- 影片工作室（/video-studio）
- 背景任務中心（/background-tasks）
- 導演 AI（/director）：整支短片的分鏡規劃
`,
    tags: ["自媒體", "短影音", "圖生影", "Kling", "入門"],
    difficulty: "beginner",
    readingMinutes: 10,
    publishedAt: "2026-07-02T00:00:00Z",
    updatedAt: "2026-07-02T00:00:00Z",
  ...
  {
    id: "scenario-educator-voice",
    category: "getting-started",
    title: "教育者 × 音：3 分鐘做出課程旁白",
    summary: "上手路徑：課程講稿→TTS 旁白（中文）→選配聲音克隆固定音色→配輕音樂。常見卡關：不知平台能做語音；怕聲音克隆很複雜；不知 Qwen TTS 中文最佳。推薦功能：ProStudio 語音合成（Qwen TTS）、音樂生成（純音樂模式）。",
    content: `# 教育者 × 音：3 分鐘做出課程旁白

## 🎓 這份教材給誰
你要做線上課程或教學影片，需要穩定、清晰的中文旁白，不想自己錄音重錄十次。

## 🚀 上手路徑

### 第 1 步：準備課程講稿
把要唸的段落整理成純文字（一段 100–300 字最順）。

### 第 2 步：TTS 生成中文旁白
到音樂配音創作室（/pro-studio）選「AI 語音合成」，中文選 Qwen TTS，貼上講稿生成。
> 你會學到：Qwen TTS 是平台上中文表現最佳的選擇；語速與停頓可以用標點控制。

### 第 3 步（選配）：聲音克隆固定音色
上傳 10–30 秒你的聲音樣本，之後整門課都用同一個「你的聲音」。
> 你會學到：聲音克隆只要一段樣本，不是想像中的大工程。

### 第 4 步：配輕音樂
用音樂生成（純音樂模式）做一段輕背景樂，墊在旁白下面。
> 你會學到：描述「soft ambient piano, no vocals, calm study mood」就能得到不搶戲的配樂。

## 🧯 常見卡關與自救
- **不知道平台能做語音** → /pro-studio 的 8 大分類裡就有 TTS、聲音克隆、音效。
- **怕聲音克隆很複雜** → 一段樣本＋一個名字就完成；先用預設聲音也完全夠用。
- **旁白聽起來太機械** → 講稿加標點斷句、拆短句，效果立刻自然。

## 🔗 推薦功能
- 音樂配音創作室（/pro-studio）：Qwen TTS＋聲音克隆＋音樂生成
`,
    tags: ["教育者", "TTS", "旁白", "聲音克隆", "入門"],
    difficulty: "beginner",
    readingMinutes: 6,
    publishedAt: "2026-07-02T00:00:00Z",
    updatedAt: "2026-07-02T00:00:00Z",
  ...
  {
    id: "scenario-freelancer-delivery",
    category: "getting-started",
    title: "接案者 × 圖影：5 步交付客戶不跑稿",
    summary: "上手路徑：定角色→存保險庫→LoRA 定版→多稿比稿→交付規格。常見卡關：客戶改稿後風格跑掉；多稿之間角色不一致；交付尺寸一張張手裁。推薦功能：一致性保險庫（Vault）、LoRA 四鎖、多尺寸匯出（社群圖像台）。",
    content: `# 接案者 × 圖影：5 步交付客戶不跑稿

## 💼 這份教材給誰
你接商業案（吉祥物、插畫、KV），最怕客戶改稿改到風格跑掉、比稿三張長三樣。

## 🚀 上手路徑（5 步）

### 第 1 步：定角色
到圖片創作室（/image-studio），用「角色定版三視圖＋固定色票」提示生出定版圖。
> 你會學到：定版圖是之後所有改稿與訓練的唯一基準。

### 第 2 步：存入保險庫
在生成結果點「存入保險庫」（/vault），type 選 character，加上案子標籤。
> 你會學到：之後每次生圖都引用同一個角色，不必重新描述。

### 第 3 步：LoRA 定版
到 LoRA 訓練室（/lora-trainer）用定版圖訓練角色 LoRA，開啟四鎖（臉／髮／裝／配件）。
> 你會學到：LoRA＋四鎖讓角色跨圖跨影都長一樣。

### 第 4 步：多稿比稿
回圖片創作室引用保險庫角色，只改構圖描述生 A／B／C 三稿。
> 你會學到：鎖住角色只換構圖，客戶挑的是排法，不是三個不同角色。

### 第 5 步：交付規格
到社群圖像台（/social/studio），把定稿設為主圖後用多尺寸匯出，一次輸出 1:1、9:16、16:9。
> 你會學到：交付尺寸不必一張張手裁，構圖和留白會自動照顧。

## 🧯 常見卡關與自救
- **客戶說改一點點，整張風格跑掉** → 別重寫提示詞；回保險庫引用定版角色，只描述要改的那一處。
- **三稿角色長得不一樣** → 先訓練角色 LoRA 並開四鎖再生比稿。
- **交付前才發現要五種尺寸** → 社群圖像台的多尺寸匯出一次輸出所有交付版本。

## 🔗 推薦功能
- 一致性保險庫（/vault）
- LoRA 四鎖（/lora-trainer）
- 多尺寸匯出（/social/studio）
`,
    tags: ["接案者", "角色定版", "LoRA", "比稿", "交付", "入門"],
    difficulty: "beginner",
    readingMinutes: 10,
    publishedAt: "2026-07-02T00:00:00Z",
    updatedAt: "2026-07-02T00:00:00Z",
  ...
  {
    id: "scenario-editor-batch",
    category: "getting-started",
    title: "內容編輯 × 圖文：一稿多平台、版本不再亂",
    summary: "上手路徑：定調性→一稿多平台→系列插圖統風→版本比對→收藏入庫。常見卡關：逐張重寫提示太慢；版本一多就亂、找不到定稿。推薦功能：提示詞庫、靈感積木、批次分鏡。",
    content: `# 內容編輯 × 圖文：一稿多平台、版本不再亂

## ✍️ 這份教材給誰
你管理刊物或品牌帳號的內容產線，要一稿改多平台、系列插圖統一風格、版本不失控。

## 🚀 上手路徑（5 步）

### 第 1 步：定調性
打開提示詞庫（/prompt-library），從公共範本挑一份調性提示，改成你們刊物的版本（聲音、讀者、句長、禁用詞）。
> 你會學到：調性寫成規則後，每篇改寫都是同一把尺量出來的。

### 第 2 步：一稿多平台
到創作工作室（/studio）貼上文案，請 AI 依 IG／FB／電子報三個平台的字數與語氣規格各出一版。
> 你會學到：一次要三版並鎖同一核心賣點，三個平台講的才是同一件事。

### 第 3 步：系列插圖統風
用批次分鏡（/animation）把整個系列排出來，套同一段風格描述一次生成，每張只換主體。
> 你會學到：風格描述只寫一次，整個專題像同一位插畫家畫的。

### 第 4 步：版本比對
到生成歷史（/history）把 A／B 兩版並排，依準則表格（點開動機、清楚度、調性、風險）比對再定稿。
> 你會學到：給 AI 明確準則，比對有依據，不是憑感覺挑。

### 第 5 步：收藏入庫
把驗證有效的定稿提示存進提示收藏庫（/prompt-collection），標上專題名稱。
> 你會學到：下一期直接複用，不用翻對話紀錄。

## 🧯 常見卡關與自救
- **十張插圖逐張重寫提示** → 把風格描述存成靈感積木，每張只換主體。
- **版本 v3、final、final2 分不清** → 定稿提示收藏入庫並命名。
- **各平台規格改到調性走味** → 先用調性範本鎖住聲音，再讓 AI 依平台改格式。

## 🔗 推薦功能
- 提示詞庫（/prompt-library）＋提示收藏庫（/prompt-collection）
- 靈感積木（/studio）
- 批次分鏡（/animation）
`,
    tags: ["內容編輯", "多平台", "批量", "版本管理", "入門"],
    difficulty: "beginner",
    readingMinutes: 9,
    publishedAt: "2026-07-02T00:00:00Z",
    updatedAt: "2026-07-02T00:00:00Z",
  ...
  {
    id: "scenario-brand-consistency",
    category: "getting-started",
    title: "品牌方 × 圖影：張張都合品牌規範",
    summary: "上手路徑：建品牌 preset→主視覺→衍生成套→審核留痕。常見卡關：生成結果不合品牌規範；衍生素材品牌感深淺不一；素材沒審就發出去。推薦功能：vibe cards、一致性保險庫（場景錨點）、確認門。",
    content: `# 品牌方 × 圖影：張張都合品牌規範

## 🏷️ 這份教材給誰
你負責品牌的視覺產出，要求每張素材都合規範、衍生成套不走鐘、對外素材有審核留痕。

## 🚀 上手路徑（5 步）

### 第 1 步：建品牌 preset
到創作工作室（/studio）用 vibe card 精靈建立品牌卡（色票、材質、氛圍、禁用元素）。
> 你會學到：之後每次生成都從同一套規範出發，不用每次重講。

### 第 2 步：主視覺
套用品牌卡，到圖片創作室（/image-studio）生成留好 logo 與標語位置的活動主視覺。
> 你會學到：留白位置＋裁切相容性寫進提示，生成稿才能直接進設計流程。

### 第 3 步：衍生成套
把主視覺存入保險庫（/vault，type 選 scene），之後每張衍生（9:16、1:1、16:9）都引用這個錨點再生成。
> 你會學到：引用同一個錨點衍生，張張同一個品牌臉。

### 第 4 步：審核確認門
在導演模式（/director）送出成套需求，在確認門逐鏡檢查後再按開始。
> 你會學到：先看計畫、按了開始才生成，不會生出一堆不合規的圖。

### 第 5 步：留痕歸檔
把過審版本存入資產庫（/assets），標上活動名稱與版號。
> 你會學到：審核紀錄隨時可回溯。

## 🧯 常見卡關與自救
- **圖漂亮但不合品牌規範** → 把品牌色票和禁用元素寫進 vibe card，每次生成自動帶上。
- **衍生素材品牌感深淺不一** → 主視覺定稿後先存成保險庫錨點再衍生，變的只有尺寸和構圖。
- **素材沒審就發出去** → 對外素材都走確認門，每一步留紀錄。
...
- 確認門（/director）
`,
    tags: ["品牌方", "品牌一致", "vibe cards", "確認門", "入門"],
    difficulty: "beginner",
    readingMinutes: 10,
    publishedAt: "2026-07-02T00:00:00Z",
    updatedAt: "2026-07-02T00:00:00Z",
...
export const SEED_VIDEOS: LearnVideo[] = [
  {
    id: "video-001",
    category: "getting-started",
...
export const SEED_QUIZZES: LearnQuiz[] = [
  {
    id: "quiz-001",
    category: "getting-started",
