...
const ENGINE_PROVIDER_MAP: Record<string, string> = {
  ...
  "fal-ai/flux-2-pro/edit": "fal",
  "fal-ai/flux-pro/v1.1": "fal",
  "fal-ai/flux/dev": "fal",
  "fal-ai/stable-diffusion-v35-large": "fal",
  "fal-ai/fast-sdxl": "fal",
  "fal-ai/lora": "fal",
  "fal-ai/seedvr/upscale/image": "fal",
  "fal-ai/dwpose": "fal",
  // ── 圖像轉3D（Fal.ai） ──
  "fal-ai/trellis-2": "fal",
  "fal-ai/trellis": "fal",
  "fal-ai/sam-3/3d-objects": "fal",
  "fal-ai/hunyuan3d-v3/image-to-3d": "fal",
  "fal-ai/hyper3d/rodin": "fal",
  "fal-ai/hunyuan_world/image-to-world": "fal",
  // ── 影片生成（Fal.ai） ──
  ...
  "fal-ai/wan-t2v": "fal",
  "fal-ai/wan-i2v": "fal",
  "fal-ai/minimax/video-01": "fal",
  "fal-ai/minimax/video-01/image-to-video": "fal",
  "fal-ai/veo3": "fal",
  "fal-ai/sora": "fal",
  "fal-ai/ltx-video-13b-distilled": "fal",
  "fal-ai/ltx-video/image-to-video": "fal",
  "fal-ai/pixverse/v4.5/image-to-video": "fal",
  "fal-ai/runway-gen4-turbo/image-to-video": "fal",
  "fal-ai/animatediff-v2v": "fal",
  "fal-ai/depthcrafter": "fal",
  "fal-ai/cammaster": "fal",
  "fal-ai/vidu/q1/reference-to-video": "fal",
  // ── 音樂生成（Fal.ai） ──
  "fal-ai/sonauto": "fal",
  "fal-ai/ace-step": "fal",
...
/** 記錄一個生成錯誤 */
export function recordErrorTrace(
  trace: Omit<ErrorTrace, "id" | "createdAt">
): ErrorTrace {
...
async function autoSearchForFix(trace: ErrorTrace): Promise<void> {
  try {
    ...
    if (results.length > 0) {
      ...
      // 建立修復提案（包含根因資訊）
      const categoryLabel =
        trace.errorCategory && trace.errorCategory !== "unknown"
...
const KNOWN_ERROR_PATTERNS: KnownErrorPattern[] = [
  // ── 速率限制 ──
  {
    ...
    rootCause:
      "API 請求過於頻繁，超過供應商的速率限制。可能是短時間內發送了太多請求，或是免費方案的配額已用盡。",
    steps: [
      {
        step: 1,
        title: "確認目前配額狀態",
        description:
          "前往 API 供應商的控制台（如 Google AI Studio、Fal.ai Dashboard），檢查目前的用量與剩餘配額。",
        action: "check",
      },
      {
        step: 2,
        title: "減少請求頻率",
        description:
          "在設定頁面調整巡檢間隔（建議 5 分鐘以上），或減少同時進行的生成任務數量。",
        action: "fix",
      },
      {
        step: 3,
        title: "升級 API 方案",
        description: "若免費配額不足，考慮升級至付費方案以獲得更高的速率限制。",
        action: "fix",
        command: "前往供應商控制台 → Billing → Upgrade Plan",
      },
      {
        step: 4,
        title: "啟用備援引擎",
        description:
          "系統已自動嘗試切換備援引擎，確認備援是否成功。若備援也失敗，可手動在大腦組態中切換引擎。",
        action: "fallback",
      },
      {
        step: 5,
        title: "驗證修復結果",
        description:
          "等待 5-10 分鐘後，回到此頁面確認錯誤是否不再出現。可使用「精準度測試」頁籤測試引擎是否恢復。",
        action: "verify",
      },
    ],
    searchHints: [
      "rate limit error fix",
      "API quota exceeded solution",
      "429 too many requests workaround",
    ],
  },
  // ── 認證失敗 ──
  {
    ...
    rootCause:
      "API Key 無效、過期或權限不足。可能是環境變數未設定，或 API Key 已被撤銷。",
    steps: [
      {
        step: 1,
        title: "檢查環境變數",
        description:
          "確認相關的 API Key 環境變數已正確設定。常見的有：GEMINI_API_KEY、FAL_API_KEY、ELEVENLABS_API_KEY、REPLICATE_API_TOKEN。",
        action: "check",
        command: "在 .env 檔案中檢查對應的 KEY 是否存在且不為空",
      },
      {
        step: 2,
        title: "驗證 API Key 有效性",
        description:
          "前往 API 供應商控制台，確認 Key 狀態為啟用中且未過期。嘗試在供應商的 API Playground 測試是否可用。",
        action: "check",
      },
      {
        step: 3,
        title: "重新產生 API Key",
        description:
          "若 Key 已過期或被撤銷，在供應商控制台重新產生新的 Key，並更新環境變數。",
        action: "fix",
        command:
      ...
      },
      {
        ...
        title: "檢查 API 權限",
        description:
          "確認 API Key 擁有所需的 API 存取權限。部分供應商需要額外啟用特定 API（如 Gemini API 需要在 Google Cloud Console 啟用）。",
        action: "fix",
      },
      {
        step: 5,
        title: "重啟伺服器並驗證",
        description:
          "更新 Key 後重啟伺服器，並使用「精準度測試」頁籤驗證 API 是否恢復正常。",
        action: "verify",
      },
    ],
    searchHints: [
  ...
  // ── 連線過多 / 連線問題 ──
  {
    ...
    rootCause:
      "網路連線問題。可能原因：一次開啟太多連線導致資源耗盡、目標伺服器暫時不可用、DNS 解析失敗、或防火牆阻擋。",
    steps: [
      {
        step: 1,
        title: "確認網路狀態",
        description:
          "檢查伺服器的網路連線是否正常。可嘗試 ping 目標 API 伺服器。",
        action: "check",
        command:
      ...
      },
      {
        ...
        title: "減少同時連線數",
        description:
          "若錯誤是「too many connections」，需減少同時進行的任務。在大腦組態中降低巡檢頻率，或限制同時進行的生成任務數。",
        action: "fix",
      },
      {
        step: 3,
        title: "檢查 DNS 設定",
        description:
          "若錯誤包含 ENOTFOUND，代表 DNS 解析失敗。檢查伺服器的 DNS 設定，或嘗試使用 Google DNS (8.8.8.8) 或 Cloudflare DNS (1.1.1.1)。",
        action: "fix",
      ...
      },
      {
        ...
        title: "等待並重試",
        description:
          "如果是暫時性的網路問題，系統會自動重試。通常等待幾分鐘後即可恢復。",
        action: "fallback",
      },
      {
        step: 5,
        title: "確認恢復",
        description: "使用「警報」頁籤的巡檢功能確認 API 端點已恢復連線。",
        action: "verify",
      },
    ],
    searchHints: [
      "too many connections fix",
      "ECONNREFUSED solution",
      "socket hang up nodejs",
    ],
  },
  // ── 逾時 ──
  {
    ...
    rootCause:
      "請求逾時。API 伺服器回應時間過長，可能是因為任務太複雜、伺服器負載過高，或網路延遲。",
    steps: [
      {
        step: 1,
        title: "確認是偶發或持續性",
        description:
          "觀察此錯誤是否只出現一次還是反覆發生。偶發逾時通常可透過重試解決。",
        action: "check",
      },
      {
        step: 2,
        title: "降低任務複雜度",
        description:
          "嘗試使用較短的提示詞、較低的解析度或較短的影片長度。複雜任務更容易逾時。",
        action: "fix",
      },
      {
        step: 3,
        title: "切換到更快的模型",
        description:
          "考慮從 Pro 模型切換到 Flash 模型（如 gemini-2.5-flash），Flash 模型回應更快。",
        action: "fix",
      },
      {
        step: 4,
        title: "檢查伺服器負載",
        description:
          "若持續逾時，可能是 API 供應商伺服器過載。查看供應商的狀態頁面了解是否有服務中斷。",
        action: "check",
        command:
      ...
      },
      {
        step: 5,
        title: "驗證修復",
        description: "調整後重新提交任務，確認不再出現逾時錯誤。",
        action: "verify",
      },
    ],
    searchHints: [
  ...
  // ── 缺失 API / 端點不存在 ──
  {
    ...
    rootCause:
      "API 端點或模型不存在。可能是模型名稱拼寫錯誤、API 版本已更新、或該模型已被供應商下架。",
    steps: [
      {
        step: 1,
        title: "確認模型名稱",
        description:
          "檢查大腦組態中設定的模型名稱是否正確。對照供應商的文件確認最新的模型 ID。",
        action: "check",
      },
      {
        step: 2,
        title: "檢查 API 版本",
        description:
          "供應商可能已更新 API 版本。確認使用的 API URL 是否為最新版本。",
        action: "check",
        command:
      ...
      },
      {
        ...
        title: "更新模型設定",
        description:
          "在大腦組態頁面中，將引擎切換為可用的模型。可使用模型目錄查看所有支援的模型。",
        action: "fix",
      },
      {
        step: 4,
        title: "啟用所需的 API",
        description:
          "部分 API 需要在供應商控制台手動啟用。前往控制台確認已啟用所需的 API 服務。",
        action: "fix",
        command: "Google Cloud Console → APIs & Services → Enable APIs",
      },
      {
        step: 5,
        title: "測試新設定",
        description: "更新設定後，使用「精準度測試」驗證新模型是否正常運作。",
        action: "verify",
      },
    ],
    searchHints: [
      "API model not found fix",
      "404 endpoint not exist",
      "API not enabled solution",
    ],
  },
  // ── 連結中斷 / 資源不存在 ──
  {
    ...
    rootCause:
      "資源連結中斷或已失效。可能是 CDN 快取過期、遠端資源已被移除、或 URL 已變更。",
    steps: [
      {
        step: 1,
        title: "確認資源 URL",
        description:
          "檢查錯誤訊息中提到的 URL 是否可正常訪問。在瀏覽器中直接開啟測試。",
        action: "check",
      },
      {
        step: 2,
        title: "清除快取",
        description: "若使用了 CDN 或快取代理，嘗試清除快取並重新載入。",
        action: "fix",
      },
      {
        step: 3,
        title: "更新資源連結",
        description: "若資源已移動，更新系統中儲存的 URL 到新的位址。",
        action: "fix",
      },
      {
        step: 4,
        title: "確認恢復",
        description: "重新提交任務，確認資源可正常訪問。",
        action: "verify",
      },
    ],
    searchHints: [
      "broken link fix",
      "resource unavailable CDN",
      "410 gone error",
    ],
  },
  // ── 參數驗證錯誤 ──
  {
    ...
    rootCause:
      "API 請求參數不正確。可能是輸入值超出範圍、缺少必填欄位、或格式不符合要求。",
    steps: [
      {
        step: 1,
        title: "閱讀錯誤訊息",
        description: "錯誤訊息通常會指出具體哪個參數有問題。仔細閱讀錯誤詳情。",
        action: "check",
      },
      {
        step: 2,
      ...
      },
      {
        ...
        title: "修正輸入",
        description:
          "根據錯誤訊息調整輸入參數。常見問題：提示詞太長、含有不支援的特殊字元、圖片格式不正確。",
        action: "fix",
      },
      {
        step: 4,
        title: "重新提交",
        description: "修正參數後重新提交任務。",
        action: "verify",
      },
    ],
    searchHints: [
  ...
  // ── 伺服器內部錯誤 ──
  {
    ...
    rootCause:
      "API 供應商的伺服器發生內部錯誤。這通常不是使用者端的問題，而是供應商服務暫時異常。",
    steps: [
      {
        step: 1,
        title: "檢查供應商狀態頁",
        description:
          "查看 API 供應商的服務狀態頁面，確認是否有已知的服務中斷事件。",
        action: "check",
        command:
      ...
      },
      {
        ...
        title: "等待恢復",
        description:
          "伺服器錯誤通常會在短時間內恢復。建議等待 10-30 分鐘後重試。",
        action: "fallback",
      },
      {
        step: 3,
        title: "使用備援引擎",
        description: "若等待後仍未恢復，可在大腦組態中手動切換到備援引擎。",
        action: "fallback",
      },
      {
        step: 4,
        title: "回報問題",
        description: "若超過 1 小時仍未恢復，建議向供應商回報問題。",
        action: "fix",
      },
      {
        step: 5,
        title: "確認恢復",
        description: "伺服器恢復後，使用「精準度測試」驗證功能是否正常。",
        action: "verify",
      },
    ],
    searchHints: [
      ...
      "502 bad gateway fix",
      "service unavailable workaround",
    ],
  },
  // ── 配額用盡 ──
  {
    ...
    steps: [
      {
        step: 1,
        title: "查看帳戶用量",
        description: "登入 API 供應商控制台，檢查目前的用量統計和剩餘配額。",
        action: "check",
      },
      {
        step: 2,
        title: "確認帳單狀態",
        description: "確認帳戶的付款方式有效且餘額充足。",
        action: "check",
      },
      {
        step: 3,
        title: "升級方案或充值",
        description: "若配額不足，考慮升級到更高的方案或充值帳戶餘額。",
        action: "fix",
        command: "前往供應商 Dashboard → Billing → Upgrade 或 Add Credits",
      },
      {
        step: 4,
        title: "暫時使用備援",
        description: "在等待配額恢復期間，可切換到其他供應商的替代引擎。",
        action: "fallback",
      },
      {
        step: 5,
        title: "設定用量警告",
        description: "建議在供應商控制台設定用量警告，避免再次耗盡配額。",
        action: "verify",
      },
    ],
    searchHints: [
      "API quota exceeded solution",
      "billing payment required fix",
      "credit insufficient API",
    ],
  },
  // ── 模型不可用 ──
  {
    ...
    rootCause:
      "所選模型目前不可用。可能是模型已被棄用、正在維護、過載，或在目前區域不支援。",
    steps: [
      {
        step: 1,
        title: "確認模型狀態",
        description:
          "查看供應商文件確認模型是否仍在支援中。部分模型可能已被新版本取代。",
        action: "check",
      },
      {
        step: 2,
        title: "切換替代模型",
        description:
          "在大腦組態中切換到可用的替代模型。查看模型目錄找到合適的替代品。",
        action: "fix",
      },
      {
        step: 3,
        title: "檢查區域限制",
        description:
          "部分模型僅在特定區域可用。確認帳戶所在區域是否支援該模型。",
        action: "check",
      },
      {
        step: 4,
        title: "等待恢復",
        description: "若模型因過載暫時不可用，等待幾分鐘後重試。",
        action: "fallback",
      },
      {
        step: 5,
        title: "驗證",
        description: "切換模型後，使用「精準度測試」驗證新模型是否正常運作。",
        action: "verify",
      },
    ],
    searchHints: [
      "model unavailable alternative",
      "deprecated model replacement",
      "model overloaded fix",
    ],
  },
];
...
function buildSearchQueries(
  ...
  switch (category) {
    ...
    default:
      queries.push(
        `${trace.engine} ${trace.errorMessage.slice(0, 60)} solution`
      );
  }
...
export function diagnoseError(traceId: string): ErrorDiagnosis | null {
  ...
  if (matched) {
    ...
    if (relatedTraceIds.length >= MIN_RELATED_TRACES_FOR_BOOST) {
      confidence = Math.min(
        MAX_CONFIDENCE,
        confidence + relatedTraceIds.length * CONFIDENCE_BOOST_PER_TRACE
      );
    }
  } else {
    ...
    steps = [
      {
        step: 1,
        title: "閱讀錯誤訊息",
        description: "仔細閱讀完整的錯誤訊息，記下關鍵的錯誤碼和錯誤描述。",
        action: "check",
      },
      {
        step: 2,
        title: "搜尋解決方案",
        description:
          "使用下方的「爬網研究」功能搜尋錯誤訊息，尋找他人的解決經驗。",
        action: "check",
      },
      {
        step: 3,
        title: "檢查 API 設定",
        description: "在「大腦組態」頁籤確認所有 API Key 和模型設定正確。",
        action: "fix",
      },
      {
        step: 4,
        title: "嘗試備援引擎",
        description: "切換到備援引擎測試是否正常運作。",
        action: "fallback",
      },
      {
        step: 5,
        title: "聯繫支援",
        description: "若以上步驟皆無效，建議記錄完整錯誤訊息並聯繫技術支援。",
        action: "verify",
      },
    ];
  }
      ...
      if (ghRes.ok) {
        ...
        for (const repo of (ghData.items ?? []).slice(0, maxResults)) {
          const item: WebResearchResult = {
            ...
            relevance: Math.min(
              90,
              50 + Math.floor(repo.stargazers_count / 100)
            ),
            addedToLearnHub: false,
            createdAt: Date.now(),
          };
  ...
  try {
    ...
    if (!apiKey) {
      ...
      if (!res.ok) {
        ...
        score = 0;
        suggestions.push(
          `引擎 ${engine} 回傳錯誤，建議檢查 API Key 或切換引擎`
        );
      } else {
        const data = (await res.json()) as {
          candidates?: Array<{
        ...
        actualResult =
          data.candidates?.[0]?.content?.parts?.[0]?.text ?? "[無回應內容]";
        if (actualResult === "[無回應內容]") {
          console.warn(
            `[BrainAutoRepair] 精準度測試: ${engine} 回傳非預期結構`,
        ...
        switch (testType) {
          case "response_quality":
            ...
            if (actualResult.length > 10 && actualResult.length < 500)
              score = 90;
            break;
          case "latency":
            ...
            if (latencyMs > 5000)
              suggestions.push(
                `延遲 ${latencyMs}ms 偏高，建議切換至 Flash 模型`
              );
            break;
          case "consistency":
            try {
              ...
              score = 30;
              suggestions.push(
                "回應非合法 JSON，建議調整 system prompt 或溫度參數"
              );
            }
            break;
          case "error_rate":
 ...
 * 3. 記錄警報
 */
export async function runHealthPatrol(): Promise<{
  checked: number;
  alerts: number;
}> {
  ...
  return {
    ...
    recentTestScore:
      accuracyTests.length > 0
        ? Math.round(
            accuracyTests.slice(0, 10).reduce((sum, t) => sum + t.score, 0) /
