...
export const MODEL_PRICING_CATALOG: Record<string, ModelPricing> = {
  ...
  // 與 canonical fal-ai/wan-t2v 對齊（同一個 fal 端點），避免 UI 與 router 顯示不同積分
  ...
  // 與 canonical fal-ai/wan-i2v 對齊（同一個 fal 端點），避免 UI 與 router 顯示不同積分
  ...
  // ── Perplexity Sonar 系列（原生 API；無 PERPLEXITY_API_KEY 時自動走
  //    OpenRouter 同名 perplexity/sonar-* model id） ──
