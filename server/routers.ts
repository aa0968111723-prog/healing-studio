...
你必須回傳 JSON，包含一個 chips 陣列。`,
  ...
  ai: router({
    ...
    registryList: adminProcedure
      ...
      .query(({ input }) => {
        return orbToolRegistryCenter.list(input?.includeDisabled ?? true);
      }),

    registryUpsert: adminProcedure
      .input(
        z.object({
      ...
      .mutation(({ input }) => {
        const tool = orbToolRegistryCenter.upsert(input);
        return { tool };
      }),

    registrySetEnabled: adminProcedure
      .input(
        z.object({
          name: z.string().min(2).max(64),
          enabled: z.boolean(),
      ...
      .mutation(({ input }) => {
        const tool = orbToolRegistryCenter.setEnabled(input.name, input.enabled);
        return { tool };
      }),

    registryDelete: adminProcedure
      .input(
        z.object({
          name: z.string().min(2).max(64),
        })
      ...
      .mutation(({ input }) => {
        const ok = orbToolRegistryCenter.remove(input.name);
        return { ok };
      }),
    ...
    reportTaskStep: brainProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (input.ok) {
          const tools = orbToolRegistryCenter.activeTools();
    ...
    chat: brainProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        const registeredTools = orbToolRegistryCenter.activeTools();
