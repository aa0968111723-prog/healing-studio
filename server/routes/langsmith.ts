...
langsmithRouter.get("/api/langsmith/stats", async (_req: Request, res: Response) => {
  const apiKey = process.env.LANGSMITH_API_KEY;
  const projectName = process.env.LANGSMITH_PROJECT ?? "default";

  if (!apiKey) {
    res.status(503).json({
      error: "LANGSMITH_API_KEY is not configured. Set it in your environment variables. Get yours at https://smith.langchain.com.",
    });
    return;
  }
  ...
  try {
    ...
    const errorCount = runs.filter((r) => r.error != null || r.status === "error").length;
    const errorRate = totalRuns > 0 ? (errorCount / totalRuns) * 100 : 0;
    const validLatencies = runs.map((r) => r.latency).filter((l): l is number => l != null);
    const avgLatency =
    ...
    res.status(500).json({ error: `Failed to fetch LangSmith stats: ${message}` });
  }
});
