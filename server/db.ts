...
export async function upsertUser(user: InsertUser): Promise<void> {
  ...
  try {
    // Explicitly omit `id` — it is an auto-increment primary key and must
    // never appear in the INSERT column list, even as DEFAULT.
    const { id: _id, ...userWithoutId } = user as InsertUser & { id?: number };
    const values: Omit<InsertUser, "id"> & { openId: string } = { openId: userWithoutId.openId! };
