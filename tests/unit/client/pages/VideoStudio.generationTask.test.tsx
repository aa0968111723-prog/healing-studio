...
/**
 * AIDV-899: VideoStudio 模型選擇狀態收斂到 useGenerationTask 後的關鍵路徑測試。
 *
 ...
 * 本測試因此「錨定頁面真實原始碼」：直接解析 VideoStudio.tsx，
 * 對五個採用點逐一斷言 initialModelId 字面值與 ?? fallback 字面值
 * 相等且等於預期預設模型，並用真實字面值驅動 hook 驗證初始行為。
...
/** 五個分頁的預期預設模型（與收斂前各分頁 useState 初始值一致）。 */
const expectedTabs: Array<[tab: string, rawVar: string, defaultModel: string]> = [
  ["t2v", "activeT2VModelRaw", "kling"],
  ["i2v", "activeI2VModelRaw", "kling"],
  ["v2v", "activeV2VModelRaw", "wan"],
  ["enhance", "activeEnhanceModelRaw", "upscale"],
  ["control", "activeControlModelRaw", "cam"],
];

describe("AIDV-899 VideoStudio useGenerationTask 採用點（錨定頁面原始碼）", () => {
  it("頁面恰有 5 個 useGenerationTask 呼叫且全部可被解析", () => {
