...
>> = [
  ...
  },
  {
    id: "critic",
    displayName: "品品",
    description: "結構化評審：跑 rubric 打分、給可貼的改寫、比較迭代、智慧交棒",
    modality: "general",
  ...
  },
  {
    ...
    knowledgeDomains: ["cost estimation", "quota / usage tracking", "subscription tier", "model pricing", "workflow cost rollup", "month-end forecasting"],
    useCases: ["這次會花多少", "本月用量還剩多少", "有沒有更便宜的做法", "推薦升降級時機", "整條工作流總共多少點", "照這節奏到月底會花多少"],
    chain: ["accountant"],
    requiresPage: false,
  },
  {
  ...
  },
  {
    ...
    tools: [
      ...
      // 自主能力工具 (autonomy): 讓總總能像 AI agent 一樣主動分解 / 推薦 / 重試 / 監控 SLA
      "orchestrator.decomposeGoal",
      "orchestrator.recommendSpirit",
      "orchestrator.analyzeBottlenecks",
      "orchestrator.retryWithFallback",
      "orchestrator.setDeadline",
    ],
    knowledgeDomains: ["agent orchestration", "task queue", "team workload", "spirit monitoring", "task delegation", "goal decomposition", "load balancing", "bottleneck analysis", "failure recovery", "SLA tracking"],
    useCases: ["團隊狀態總覽", "誰負責什麼", "下一棒交給誰", "監控精靈進度", "處理執行失敗", "把目標拆成有序步驟", "依負載挑最適精靈", "找出卡住的任務", "失敗時自動切替代精靈", "設定任務截止時間"],
  ...
  },
  {
    id: "settings-detail",
    displayName: "細細",
    description: "設定與偏好的 AI agent：讀、改、預覽 diff、批次套用、預設組合、偵測設定衝突、主動推薦優化",
    modality: "general",
    recommendedPages: ["/settings", "/agent"],
    tools: [
      // CRUD 基礎
      "settingsDetail.getPreferences",
      "settingsDetail.updatePreference",
      "settingsDetail.explainSetting",
      "settingsDetail.getAllSettings",
      "settingsDetail.validatePreference",
      // 自主能力（agent-like）
      "settingsDetail.bulkUpdate",
      "settingsDetail.previewDiff",
      "settingsDetail.applyPreset",
      "settingsDetail.listPresets",
      "settingsDetail.resetPreference",
      "settingsDetail.searchSettings",
      "settingsDetail.detectInconsistencies",
      "settingsDetail.recommendOptimizations",
    ],
    knowledgeDomains: [
      "settings surface",
      "preferences",
    ...
    useCases: [
      ...
      "改密碼 / 金鑰前置",
      "微調預設模型",
      "套用「謹慎 / 平衡 / 自動駕駛 / 安靜 / 專注」整組設定",
      "預覽偏好變更的 diff 再決定要不要套",
      "找出設定衝突（例如同一工具被自動核准又被封鎖）",
      "根據目前狀態主動建議調整",
      "用自然語意搜「我想關掉自動麥克風」找到對應 key",
      "把某個偏好重設成預設值",
    ],
