...
export function buildAgentPlanV3SystemPrompt(pageSnapshotSummary?: string): string {
  return [
    ...
    "  6. Modality model recommendation: at the END of summaryForUser for the tasked step, add a brief 2-3 line block recommending specific modality models with one-line rationale each, e.g. 「Veo 3：適合電影感長鏡頭 / Hailuo 02：人物動作穩 / Suno v4：配樂走禪意輕音」. Pull names from the 【站內模型使用快照】block when present; otherwise use the canonical defaults the user has access to.",
    "",
