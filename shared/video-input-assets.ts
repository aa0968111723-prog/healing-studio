 ...
 *
 * 範圍（放行令 triage 結論）：本契約涵蓋 image 與 audio（背景音樂，僅登記 URL）。
 * 注意：image 素材目前「僅登記進契約」——selectFalImageUrl 已備好選取邏輯，但
 * i2v 生成路徑的 image_url 注入尚未接線（VideoStudio 的 runX 提交路徑拿不到
 * videoProject context，硬接需動 AIDV-970 剛收斂的提交骨架），列為第二階段
 * follow-up 另卡處理。audio 的 FFmpeg 混音同為第二階段 follow-up，本檔不做。
 */
...
/**
 * 單一素材檔案大小上限（per-type）。
 * 鏡射 server/uploadRoute.ts 的 PER_KIND_MAX_BYTES（image 10MB / audio 20MB）——
 * 上傳端點就是這個上限，契約層若宣稱更大只會讓使用者在上傳時才被打回。
 * 改 uploadRoute 的 PER_KIND_MAX_BYTES 時必須同步這裡。
 */
export const MAX_INPUT_ASSET_BYTES_BY_TYPE = {
  image: 10 * 1024 * 1024,
  audio: 20 * 1024 * 1024,
} as const;
 ...
 *
 * url 加 isSafeExternalUrl refine（shared/safe-url.ts）：素材 URL 會被下游
 * （fal 派發 / 未來混音）抓取，https-only + 私網/metadata 封鎖擋掉 SSRF 注入。
 */
export const videoInputAssetSchema = z
  .object({
    ...
    url: z
      ...
      .url()
      .max(2048)
      .refine(isSafeExternalUrl, "僅接受 https URL 且不可指向內部網路"),
 ...
 *
 * 純函式、與 fal 派發解耦。注意：i2v 端的實際注入尚未接線（見檔頭 follow-up 聲明）。
 */
...
/** 把可能帶參數的 MIME（如 "image/png; charset=binary"）正規化成小寫主型別。 */
...
export function isAllowedImageMime(mime: string): boolean {
  return (ALLOWED_IMAGE_MIME_TYPES as readonly string[]).includes(normalizeMime(mime));
}

/** 是否為允許的音訊 MIME。 */
export function isAllowedAudioMime(mime: string): boolean {
  return (ALLOWED_AUDIO_MIME_TYPES as readonly string[]).includes(normalizeMime(mime));
}

/** 副檔名 → 模態 fallback（瀏覽器/OS 可能回空 MIME，如 Windows 上的 .wav）。 */
const EXTENSION_TO_TYPE: Record<string, VideoInputAssetType> = {
  png: "image",
  jpg: "image",
  jpeg: "image",
  mp3: "audio",
 ...
 * MIME 為空或不可辨識時，若有 fileName 則依副檔名 fallback
 * （.png/.jpg/.jpeg → image、.mp3/.wav → audio，其餘 null）。
 * 純函式，前端上傳前與後端契約層皆可共用。
 */
export function inferAssetTypeFromMime(
  mime: string,
  fileName?: string,
): VideoInputAssetType | null {
...
export interface AssetFileValidationInput {
  mime: string;
  sizeBytes: number;
  /** MIME 為空/不可辨識時，用來做副檔名 fallback（可省略）。 */
  fileName?: string;
}

export type AssetFileValidationResult =
  | { ok: true; type: VideoInputAssetType }
  | { ok: false; reason: "unsupported_type" | "too_large" | "empty_file" };

/**
 * 上傳前的檔案驗證：型別在白名單內、非空檔、且 ≤ 該模態上限
 * （image 10MB / audio 20MB，鏡射 uploadRoute 的 PER_KIND_MAX_BYTES）。
 * 純函式，前端拖放時即時擋掉不合法檔案，避免無謂上傳。
 */
export function validateAssetFile(
  input: AssetFileValidationInput,
): AssetFileValidationResult {
  const type = inferAssetTypeFromMime(input.mime, input.fileName);
  if (!type) return { ok: false, reason: "unsupported_type" };
  if (input.sizeBytes <= 0) return { ok: false, reason: "empty_file" };
  if (input.sizeBytes > MAX_INPUT_ASSET_BYTES_BY_TYPE[type]) {
