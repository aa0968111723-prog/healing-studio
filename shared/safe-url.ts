/**
 * safe-url.ts — SSRF 安全外部 URL 判定（AIDV-201 / AIDV-270）
 *
 * 原位於 server/utils/validateSafeUrl.ts；AIDV-270 把純函式搬進 shared，
 * 讓 /video 輸入素材契約（shared/video-input-assets.ts）在前後端共用同一套
 * 私網/metadata 封鎖規則。server/utils/validateSafeUrl.ts 改為自此匯入並
 * re-export（zod schema 仍留在 server 端，對外簽名零變化）。
 *
