...
## 工作表「做完的感覺」（自檢，過設計門前先勾）
> 工作表不是「填滿」就完，是「**想清楚了**」才完。把感覺翻成清單，設計門不靠手感。
> **一句話**：整張讀一遍**沒有一欄讓你心虛**，敢拿去跟 Bruce 對賭「照這樣做就會過」＝做完。

七個訊號（全中才算完）：
1. **每欄都是實話**——沒 `TODO`／`待補`／空白。
2. **範圍一句話**——講得完，不夾「順便」。
3. **重用是查過的**——「重用既有」寫具體 procedure／元件名（搜過碼），真沒有才寫「新建（原因）」。
4. **驗收別人能驗**——第三人不靠你解釋就能判通過／不通過（非「做好」「正常」）。
5. **設計門一翻兩瞪眼**——`自動通過 ✓` 或「待 Bruce 拍板（寫明原因）」，不留「應該可以」。
6. **下一步唯一**——只有一條路：進階2 或 停下等拍板。
7. **風險先寫在前**——避雷是開工前就寫，不是寫程式才發現。

**收尾自問**：『如果 Bruce 只看這張表、不問我半句，他會放行嗎？』——答得出「會」且說得出理由，才轉「就緒」過設計門。
