...
try {
  const resp = await fetch("https://api.replicate.com/v1/models/ostris/flux-dev-lora-trainer", {
...
try {
  ...
  const data = await resp.json();
  if (resp.ok) {
    console.log(`✅ Predictions API accessible. Recent predictions: ${data.results?.length || 0}`);
    if (data.results && data.results.length > 0) {
      const latest = data.results[0];
      console.log(`   Latest: ${latest.model} | status: ${latest.status} | created: ${latest.created_at}`);
    }
  } else {
