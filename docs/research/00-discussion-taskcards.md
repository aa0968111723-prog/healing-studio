...
## 10.19 TC 波測試覆蓋卡(修復必讀,詳見 `TC0-test-coverage-map.md`)

### 卡 TC-01 ·【修復前必讀】既有測試把 bug/漏洞鎖成「正確規格」
- 群組:測試/修復風險 ｜ 驗證:已確認(TC 波,逐一 Read 測試檔行號)
- 現況:修下列頂級發現時,**CI 會反抗**,因為既有測試把錯誤行為當正解,必須**同一 PR 改測試**:
  - **B-27**(updateQuotaJson 免費點數):`phase2.test.ts:315-326`、`phase3-audit.test.ts:308-332` 鎖「一般使用者 image:10 應成功」為正確。
  - **B-19**(submitStudioJob 不寫 costPoints):`refundStatus.test.ts:63-72` 鎖「無 costPoints→不退款」為正確。
- **待決策**:修這兩條務必連帶更新對應測試,並補「上限/角色拒絕」「退款端到端」正向測試。

### 卡 TC-02 ·【回歸網缺失】7 個頂級計費發現零回歸測試
- 群組:測試 ｜ 驗證:已確認(TC 波)
- 現況:B-01(ProStudio ~20 async 失敗退款)、B-03(ai.chat 計費)、B-16(models 訓練限流/計費)、B-32(Stripe webhook entitlement)、**X3-C2(invokeLLM 成本回寫=雙向零覆蓋黑洞)** 完全無測試→修或再壞 CI 都看不到。
- **待決策**:第 0/1 波修復時同批補回歸測試。優先 3 類(TC0):①測試鎖死 bug 者(B-19/B-27)同步改;②雙向零覆蓋計費黑洞(B-03/X3-C2/B-32)新增釘住測試;③B-01 ProStudio 退款先寫一代表模態測試作範本再橫向鋪。
- 待再查:北極星一條龍**端到端測試**是否存在(本輪未系統搜 e2e/integration,標需再查)。
