...
| 顯式共享 SSOT 表 | `drizzle/schema.ts` `resourceShares` + `drizzle/0078_resource_shares.sql` | 泛型 junction（resourceType／resourceId／sharedWithType user\|team／sharedWithId／role viewer\|editor／sharedByUserId）。journal idx 82（改號避開 #940 cost_ledger 的 0077）、information_schema 守門、一句一 breakpoint。 |
