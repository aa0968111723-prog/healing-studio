...
export type WorkspaceAction =
  ...
  | {
      type: "SET_BLOCKS";
      payload: { modality: Modality; blocks: StructuredBlock[] };
    }
  | {
      type: "UPDATE_BLOCK";
      payload: {
        modality: Modality;
        blockId: string;
        changes: Partial<StructuredBlock>;
      };
    }
  | { type: "SET_PROMPT_STRENGTH"; payload: PromptStrengthLevel }
  | {
      type: "SET_ADVANCED_PROMPT";
      payload: { modality: Modality; prompt: string };
    }
  | {
      type: "SET_ADVANCED_PROMPT_OVERRIDE";
      payload: { modality: Modality; override: boolean };
    }
  | { type: "SET_COMPILED_PROMPT"; payload: string }
  | {
      type: "ADD_REFERENCE";
      payload: { modality: Modality; reference: ReferenceItem };
    }
  | {
      type: "REMOVE_REFERENCE";
      payload: { modality: Modality; referenceId: string };
    }
  | {
...
export function getDefaultThoughtIslands(modality: Modality): ThoughtIsland[] {
  return (defaultThoughtIslands[modality] || defaultThoughtIslands.image).map(
    i => ({ ...i })
  );
}
  ...
  switch (modality) {
    ...
    case "voice":
      return [
        ...
        block(
          "voi-voiceConsistency",
          "voiceConsistency",
          "Voice Consistency",
          "control"
        ),
        block(
          "voi-negativeConstraints",
          "negativeConstraints",
          "Negative Constraints",
          "control"
        ),
...
const WorkspaceContext = createContext<
  | { state: WorkspaceState; dispatch: React.Dispatch<WorkspaceAction> }
  | undefined
>(undefined);
