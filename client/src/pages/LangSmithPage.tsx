...
function ConnectionBadge({ connected, message }: { connected: boolean; message: string }) {
  return (
    <Badge
...
function OverviewTab() {
    ...
    return (
      <div className="space-y-4">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[1, 2, 3, 4].map((i) => (
            <GlassCard key={i} hover={false}><ZenSkeleton lines={3} /></GlassCard>
          ))}
        </div>
  ...
  const statCards = [
    { icon: Activity, label: "總追蹤數", value: health.total, unit: "runs", color: "bg-zen-lavender/20", textColor: "text-violet-600" },
    { icon: CheckCircle2, label: "成功率", value: `${health.successRate}%`, unit: `${health.success} 成功`, color: "bg-zen-sage/20", textColor: "text-emerald-600" },
    { icon: Clock, label: "平均延遲", value: health.avgLatencyMs, unit: "ms", color: "bg-zen-sky/20", textColor: "text-blue-600" },
    { icon: Zap, label: "Token 總數", value: health.totalTokens.toLocaleString(), unit: `$${health.totalCostUsd}`, color: "bg-zen-peach/20", textColor: "text-orange-600" },
  ];

  const errorBreakdownData = Object.entries(health.errorBreakdown).map(([name, count]) => ({
    name,
    count,
  }));
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <GlassCard hover={false}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <Badge variant="destructive" className="text-[10px] ml-auto">{health.errors} 錯誤</Badge>
          </div>
          {errorBreakdownData.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={160}>
              <BarChart data={errorBreakdownData} layout="vertical" margin={{ left: 60, right: 16, top: 4, bottom: 4 }}>
                ...
                <Bar dataKey="count" fill="#f87171" radius={[0, 4, 4, 0]} maxBarSize={20} name="次數" />
              </BarChart>
            </ResponsiveContainer>
          )}
        </GlassCard>
          ...
          {health.recentErrors.length === 0 ? (
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto">
              {health.recentErrors.map((err) => (
                <div key={err.id} className="p-2 rounded-lg bg-destructive/5 text-xs">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-destructive">{err.name}</span>
                    {err.time && <span className="text-muted-foreground">{new Date(err.time).toLocaleString("zh-TW")}</span>}
                  </div>
                  <p className="text-muted-foreground mt-0.5 line-clamp-2">{err.error}</p>
                </div>
              ))}
            </div>
          )}
...
function TracesTab() {
  ...
  return (
    <div className="space-y-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1 text-[11px] text-muted-foreground">
                      ...
                      {run.extra?.cost_usd != null && (
                        <span>${(run.extra.cost_usd as number).toFixed(5)}</span>
                      )}
                      {run.start_time && (
                        <span>{new Date(run.start_time).toLocaleString("zh-TW")}</span>
                      )}
                    </div>
                  </div>
          ...
          {runDetailQuery.isLoading ? (
            ...
            <div className="space-y-4 text-sm">
              <div className="grid grid-cols-2 gap-3">
                <div><span className="text-muted-foreground">名稱：</span>{runDetailQuery.data.name}</div>
                <div><span className="text-muted-foreground">類型：</span>{runDetailQuery.data.run_type}</div>
                <div><span className="text-muted-foreground">狀態：</span>
                  <Badge variant={runDetailQuery.data.status === "error" ? "destructive" : "secondary"} className="text-xs">
              ...
              {runDetailQuery.data.inputs && (
                <div>
                  <h4 className="text-xs font-semibold text-muted-foreground mb-1">輸入</h4>
              ...
              {runDetailQuery.data.outputs && (
                <div>
                  <h4 className="text-xs font-semibold text-muted-foreground mb-1">輸出</h4>
                  <pre className="p-3 rounded-lg bg-muted/20 text-xs overflow-x-auto max-h-40">
              ...
              {feedbackQuery.data && feedbackQuery.data.length > 0 && (
                <div>
                  <h4 className="text-xs font-semibold text-muted-foreground mb-2">回饋紀錄</h4>
                  <div className="space-y-1">
                    {feedbackQuery.data.map((fb) => (
                      <div key={fb.id} className="flex items-center gap-2 text-xs p-2 rounded-lg bg-muted/10">
                        ...
                        <span className="text-muted-foreground ml-auto">{fb.score}</span>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      {/* Feedback Dialog */}
      <Dialog open={!!feedbackRunId} onOpenChange={(open) => { if (!open) setFeedbackRunId(null); }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
          ...
          <div className="space-y-4 pt-2">
            <p className="text-xs text-muted-foreground">對此次 AI 回應進行評分：</p>
            <Textarea
              placeholder="（選填）回饋說明..."
            ...
            <div className="flex gap-3">
              <Button
                ...
                variant="outline"
                onClick={() => feedbackRunId && handleFeedback(feedbackRunId, 1)}
              ...
              <Button
                className="flex-1 rounded-xl gap-1.5"
                variant="outline"
                onClick={() => feedbackRunId && handleFeedback(feedbackRunId, 0)}
                disabled={createFeedback.isPending}
              >
...
function ComparisonTab() {
    ...
    return (
      <GlassCard>
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-2">需要至少有 LLM 追蹤紀錄才能進行對比分析。</p>
        </div>
      </GlassCard>
    );
  }
  ...
  const radarData = [
    { metric: "成功率", ...Object.fromEntries(topModels.map((m) => [m.model, m.successRate])) },
    { metric: "速度", ...Object.fromEntries(topModels.map((m) => [m.model, Math.round((1 - m.avgLatencyMs / maxLatency) * 100)])) },
    { metric: "Token效率", ...Object.fromEntries(topModels.map((m) => [m.model, Math.round((1 - m.avgTokensPerRun / maxTokens) * 100)])) },
    { metric: "成本效益", ...Object.fromEntries(topModels.map((m) => [m.model, Math.round((1 - m.avgCostPerRun / maxCost) * 100)])) },
    { metric: "品質", ...Object.fromEntries(topModels.map((m) => [m.model, m.qualityScore ?? 50])) },
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <Badge variant="secondary" className="text-[10px] ml-auto">{data.totalRuns} 總 Runs</Badge>
      </div>
            ...
            <GlassCard>
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
                ...
                <div>
                  ...
                  <p className={`font-semibold ${model.successRate >= 95 ? "text-emerald-600" : model.successRate >= 80 ? "text-amber-600" : "text-destructive"}`}>
                    {model.successRate}%
                  </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-semibold">${model.avgCostPerRun.toFixed(6)}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-semibold">
                    {model.qualityScore != null ? `${model.qualityScore}%` : "—"}
                    {model.scoredRuns > 0 && (
                      <span className="text-muted-foreground font-normal"> ({model.scoredRuns})</span>
                    )}
                  </p>
                </div>
              </div>
        ...
        <GlassCard hover={false}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={barData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              ...
              <Bar dataKey="延遲" fill="#818cf8" radius={[4, 4, 0, 0]} maxBarSize={24} />
              <Bar dataKey="使用次數" fill="#34d399" radius={[4, 4, 0, 0]} maxBarSize={24} />
            </BarChart>
          </ResponsiveContainer>
        </GlassCard>
...
function DatasetsTab() {
  const datasetsQuery = trpc.langsmith.listDatasets.useQuery(undefined, { retry: false });
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Add to dataset form */}
      <GlassCard>
        <h3 className="text-xs font-semibold text-muted-foreground mb-3">案例提取 — 將 Run 存入 Dataset</h3>
        <div className="flex items-end gap-3">
          ...
          <div className="flex-1 space-y-1">
            <label className="text-[11px] text-muted-foreground">Dataset 名稱</label>
            <Input
          ...
          <Button
            className="rounded-xl text-xs"
            size="sm"
            onClick={() => addRunMutation.mutate({ runId: addDialogRunId, datasetName: addDialogDataset })}
            disabled={!addDialogRunId.trim() || !addDialogDataset.trim() || addRunMutation.isPending}
          >
      ...
      {datasetsQuery.isLoading ? (
        <GlassCard hover={false}><ZenSkeleton lines={4} /></GlassCard>
        ...
        <GlassCard>
          <div className="flex flex-col items-center justify-center py-10 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">從 Traces 頁籤將優質 Run 存入 Dataset 開始建構數據資產。</p>
          </div>
        </GlassCard>
      ) : (
        <div className="space-y-2">
          {datasetsQuery.data.map((ds) => (
            <motion.div key={ds.id} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}>
              <GlassCard>
                <div className="flex items-center gap-3">
                  <Database className="w-4 h-4 text-muted-foreground shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate">{ds.name}</p>
                    {ds.description && <p className="text-xs text-muted-foreground mt-0.5 line-clamp-1">{ds.description}</p>}
                  </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-muted-foreground">
        篩選出用戶點讚的高品質對話，導出為 OpenAI 或 JSONL 格式，用於後續模型微調。
      </p>
      ...
      <GlassCard>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="space-y-1">
            <label className="text-[11px] text-muted-foreground">導出格式</label>
          ...
          <div className="space-y-1">
            <label className="text-[11px] text-muted-foreground">最低評分 (0-1)</label>
            <Input
              type="number"
              min={0}
              max={1}
          ...
          <div className="space-y-1">
            <label className="text-[11px] text-muted-foreground">數量上限</label>
            <Input
              type="number"
              min={1}
              max={500}
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto">
              ...
              {exportQuery.data.records.length > 10 && (
                <p className="text-xs text-muted-foreground text-center py-2">
                  ... 還有 {exportQuery.data.records.length - 10} 筆（點擊下載查看完整資料）
                </p>
              )}
            </div>
          )}
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-xs text-muted-foreground -mt-4">
        LangSmith 深度整合 — 全鏈路追蹤、生產監控、回饋收集、模型對比與微調數據導出。
      </p>
