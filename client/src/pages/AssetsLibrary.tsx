...
const typeConfig: Record<string, { icon: React.ReactNode; label: string; color: string; ext: string; mimePrefix: string }> = {
  ...
  video: { icon: <Video className="w-4 h-4" />, label: "影片", color: "bg-zen-sky/20", ext: "mp4", mimePrefix: "video/" },
  audio: { icon: <Music className="w-4 h-4" />, label: "音樂", color: "bg-zen-peach/20", ext: "mp3", mimePrefix: "audio/" },
  voice: { icon: <Mic className="w-4 h-4" />, label: "語音", color: "bg-zen-blush/20", ext: "mp3", mimePrefix: "audio/" },
  script: { icon: <FileText className="w-4 h-4" />, label: "腳本", color: "bg-zen-sage/20", ext: "txt", mimePrefix: "text/" },
  zip_bundle: { icon: <Package className="w-4 h-4" />, label: "打包", color: "bg-zen-sand/20", ext: "zip", mimePrefix: "application/zip" },
};

const ASSET_TYPES = ["all", "image", "video", "audio", "voice", "script", "zip_bundle"] as const;
...
function UploadDialog({ onSuccess }: { onSuccess: () => void }) {
  ...
  const [assetType, setAssetType] = useState<"image" | "video" | "audio" | "voice" | "script" | "zip_bundle">("image");
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    else if (file.type === "text/plain" || file.type.includes("script")) setAssetType("script");
    else if (file.type === "application/zip" || file.name.endsWith(".zip")) setAssetType("zip_bundle");
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><Upload className="w-4 h-4" /> 手動上傳資產</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          ...
          >
            ...
            {selectedFile ? (
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground/60">支援圖片、影片、音訊、文字、ZIP</p>
              </div>
            )}
          </div>
          {/* Title */}
          <div className="space-y-1.5">
            ...
            <Input value={title} onChange={e => setTitle(e.target.value)} placeholder="輸入標題..." className="h-9 text-sm rounded-lg" />
          </div>
          {/* Type */}
          <div className="space-y-1.5">
            ...
            <Select value={assetType} onValueChange={v => setAssetType(v as typeof assetType)}>
              ...
              <SelectContent>
                {(["image", "video", "audio", "voice", "script", "zip_bundle"] as const).map(t => (
                  <SelectItem key={t} value={t}>{typeConfig[t].label}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
          <Button className="w-full h-10 rounded-xl gap-2" onClick={handleUpload} disabled={uploading || !selectedFile}>
            {uploading ? <><span className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin" /> 上傳中...</> : <><Upload className="w-4 h-4" /> 上傳</>}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
...
export default function AssetsLibrary() {
  ...
  const myAssetsQuery = trpc.assets.myAssets.useQuery({ assetType: typeFilter, search: search || undefined }, { retry: false });
  const teamAssetsQuery = trpc.assets.teamAssets.useQuery({ assetType: typeFilter, search: search || undefined }, { retry: false });
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        <div className="flex gap-1">
          {ASSET_TYPES.map(t => (
            ...
            >
              {t === "all" ? <Filter className="w-3 h-3" /> : typeConfig[t]?.icon}
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
          {assets.map((asset, idx) => {
            const config = typeConfig[asset.assetType] || { icon: <Package className="w-4 h-4" />, label: asset.assetType, color: "bg-muted/20", ext: "bin", mimePrefix: "" };
                ...
                <GlassCard className="overflow-hidden group p-2 sm:p-4">
                  ...
                  <div className="aspect-square rounded-lg overflow-hidden bg-muted/20 relative mb-2 sm:mb-3">
                    {asset.fileUrl && asset.assetType === "image" ? (
                      ...
                      <video src={asset.fileUrl} className="w-full h-full object-cover" muted playsInline />
                    ) : (
                    ...
                    {!isMobile && (
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all flex items-end justify-center pb-2 opacity-0 group-hover:opacity-100">
                        <div className="flex gap-1">
                          {asset.fileUrl && (
                            <Button variant="outline" size="sm" className="rounded-lg bg-white/90 text-xs h-7 gap-1"
                          ...
                          {tab === "my" && (
                            <>
                              ...
                              <Button variant="outline" size="sm" className="rounded-lg bg-white/90 text-xs h-7 text-destructive"
                  ...
                  {isMobile && (
                    <div className="flex gap-1 mb-2 justify-end">
                      {asset.fileUrl && (
                        <button className="p-1.5 rounded-lg bg-muted/30 hover:bg-muted/60 transition-colors" title="下載"
                          onClick={() => downloadFile(asset.fileUrl!, `${asset.title.replace(/[^a-zA-Z0-9\u4e00-\u9fff]/g, "_")}.${config.ext}`)}>
                          <Download className="w-3.5 h-3.5 text-muted-foreground" />
                        </button>
                      )}
                      {asset.fileUrl && (
                        <button className="p-1.5 rounded-lg bg-muted/30 hover:bg-muted/60 transition-colors" title="開啟"
                      ...
                      {tab === "my" && (
                        <>
                          <button className="p-1.5 rounded-lg bg-muted/30 hover:bg-muted/60 transition-colors"
                          ...
                          <button className="p-1.5 rounded-lg bg-destructive/10 hover:bg-destructive/20 transition-colors" title="刪除"
                            onClick={() => deleteAsset.mutate({ id: asset.id })}>
                      ...
                      {isExpanded && (
                        ...
                        >
                          <div className="pt-2 border-t border-border/20 space-y-1.5 text-[10px] text-muted-foreground">
                            ...
                            {asset.mimeType && (
                              <div className="flex items-center gap-1"><span className="font-medium text-foreground">格式：</span><span>{asset.mimeType}</span></div>
                            )}
                            {asset.fileSizeBytes != null && (
                              <div className="flex items-center gap-1"><HardDrive className="w-2.5 h-2.5" /><span className="font-medium text-foreground">大小：</span><span>{formatBytes(asset.fileSizeBytes)}</span></div>
                            )}
                            <div className="flex items-center gap-1"><Calendar className="w-2.5 h-2.5" /><span className="font-medium text-foreground">建立：</span><span>{new Date(asset.createdAt).toLocaleString("zh-TW")}</span></div>
                            {asset.fileUrl && (
                              <Button variant="outline" size="sm" className="w-full h-7 text-xs gap-1 rounded-lg mt-2"
                                onClick={() => downloadFile(asset.fileUrl!, `${asset.title.replace(/[^a-zA-Z0-9\u4e00-\u9fff]/g, "_")}.${config.ext}`)}>
                                <Download className="w-3 h-3" /> 下載 {config.ext.toUpperCase()}
                              </Button>
                            )}
                          </div>
                        </motion.div>
