      ...
      generate: {
        ...
        submitStudioJob: {
          mutate: async (input: { provider: ProviderId }) => {
            ...
            if (failProviders.has(input.provider)) {
              const e: any = new Error(`${input.provider} upstream fail`);
...
describe("免費引擎不可用 → 不無聲跨付費扣點", () => {
  it("選 mock + mock 故障 → cost-blocked、丟 AdapterCostBlockedError、付費 provider 完全沒被呼叫", async () => {
    ...
    await expect(
      gen.generate(baseReq("mock"), (e) => events.push(e)),
...
describe("付費生成不回歸", () => {
  it("選 hf（付費）→ 照常生成，回傳 provider=hf，成本如實", async () => {
    ...
    const events: GenEvent[] = [];
    const res = await gen.generate(baseReq("hf"), (e) => events.push(e));
  ...
  it("選 hf + hf/gemini 皆故障 → fal 接手；mock 不在付費鏈中故不會被當成兜底", async () => {
    ...
    const res = await gen.generate(baseReq("hf"), () => {});
