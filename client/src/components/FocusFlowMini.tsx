    ...
    <div className="flex flex-col items-center gap-3 py-2">
      ...
      <div className="flex items-center gap-2">
        <span className="text-xs font-medium px-2 py-0.5 rounded-full" style={{
          background: pomodoroPhase === "work" ? "rgba(239,68,68,0.1)" : "rgba(34,197,94,0.1)",
          color: pomodoroPhase === "work" ? "rgb(239,68,68)" : "rgb(34,197,94)",
        }}>
      ...
      <div className="relative w-20 h-20 flex items-center justify-center">
        <svg className="absolute inset-0 w-full h-full -rotate-90" viewBox="0 0 100 100">
          <circle cx="50" cy="50" r="42" fill="none" stroke="currentColor" strokeWidth="3" className="text-gray-100" />
          <circle
            cx="50" cy="50" r="42" fill="none" stroke="currentColor" strokeWidth="4"
            strokeLinecap="round"
            strokeDasharray={`${2 * Math.PI * 42}`}
            strokeDashoffset={`${2 * Math.PI * 42 * (1 - progress / 100)}`}
            className={pomodoroPhase === "work" ? "text-red-400 transition-all duration-1000" : "text-green-400 transition-all duration-1000"}
          />
        </svg>
        <span className="text-base font-bold tabular-nums text-gray-800">{formatTime(pomodoroRemaining)}</span>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        >
          {pomodoroRunning ? <Pause className="h-3.5 w-3.5" /> : <Play className="h-3.5 w-3.5 ml-0.5" />}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col items-center gap-3 py-2">
      ...
      <div className="relative w-20 h-20 flex items-center justify-center">
        <motion.div
          ...
          style={{
            width: 56,
            height: 56,
            background: "radial-gradient(circle at 35% 35%, rgba(236,180,198,0.6), rgba(212,197,226,0.4), rgba(200,213,224,0.2))",
          }}
        />
        <div className="flex flex-col items-center z-10">
          ...
          <span className="text-base font-bold tabular-nums text-gray-800 mt-0.5">{formatTime(healingRemaining)}</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        >
          {healingRunning ? <Pause className="h-3.5 w-3.5" /> : <Play className="h-3.5 w-3.5 ml-0.5" />}
        </button>
      </div>
    </div>
  );
...
function MiniThoughts() {
  ...
  return (
    <div className="flex flex-col gap-2 py-2">
      <span className="text-[10px] text-gray-400 text-center">快速記下想法</span>
      ...
      <div className="max-h-24 overflow-y-auto space-y-1">
        {thoughts.slice(-3).map((t) => (
          <div key={t.id} className="flex items-center gap-1.5 group">
            <span className="text-[10px] text-gray-500 flex-1 truncate">{t.text}</span>
            <button
      ...
      {thoughts.length > 0 && (
        <div className="flex items-center justify-between">
          <span className="text-[10px] text-gray-400">{thoughts.length} 則想法</span>
          <button
