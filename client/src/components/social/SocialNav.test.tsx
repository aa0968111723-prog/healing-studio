 ...
 * 守住兩條路徑：
 *   · 旗標 OFF（預設）＝既有 wouter Link 子導覽＝零變化（4 條連結帶 href，未進 .aidv-kit 範圍）。
 *   · 旗標 ON       ＝design-kit 亮色暖光 SubTabs（進 .aidv-kit 範圍；tablist + 4 個 tab，標籤都在）。
...
const LABELS = ["工作台", "圖像台", "品牌庫", "發佈/精選"];

describe("SocialNav（U-6 / AIDV-96 · /social 逐殼採用第 1 片）", () => {
  it("旗標 OFF（預設）：既有 Link 子導覽，4 條帶 href、未進設計套件範圍", () => {
    flags.chrome = false;
