...
export const INSPIRATION_PRESETS: InspirationPreset[] = [
  ...
  },
  {
    ...
    emoji: "🏔️",
    mood: "超然",
    blocks: { subject: "山水", style: "水墨畫", lighting: "薄霧", color: "單色", mood: "超然" },
  },
  {
    label: "童話城堡",
    emoji: "🏰",
    mood: "奇幻",
    blocks: { subject: "城堡", style: "插畫風", lighting: "極光", color: "高飽和", mood: "奇幻" },
  },
];
...
export function InspirationQuickPanel({ onApply, className }: InspirationQuickPanelProps) {
  ...
  const handleApply = useCallback((preset: InspirationPreset) => {
  ...
  return (
    <div className={cn("space-y-3", className)}>
      ...
      <div className="flex items-center justify-between">
        <p className="text-xs font-medium text-muted-foreground/80">✨ 靈感快選</p>
        <button
              ...
              <AnimatePresence>
                {isActive && (
                  <motion.div
                    ...
                    style={{ background: "radial-gradient(circle, rgba(212,197,226,0.4), transparent)" }}
                  />
                )}
              </AnimatePresence>

              <span className="text-2xl leading-none relative z-10">{preset.emoji}</span>
