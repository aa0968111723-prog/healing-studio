...
export function VisualInspirationLibraryPanel({ world }: { world: WorldbuildingFrameworkData }) {
  ...
  return <div className="rounded-lg border p-3 space-y-2">
    ...
    {items.length===0?<div className="text-xs text-muted-foreground">還沒有視覺靈感。你可以上傳角色圖、場景圖、風格參考，或從腳本自動產生靈感草稿。</div>:<div className="grid grid-cols-1 md:grid-cols-2 gap-2">{items.map(it=><div key={it.id} className="border rounded p-2 text-xs space-y-1">{it.imageUrl?<img src={it.imageUrl} className="w-full h-24 object-cover rounded"/>:<div className="h-24 rounded bg-muted grid place-items-center">prompt-only</div>}<div className="font-medium">{it.title}</div><div>{it.kind}</div><div>{it.promptSeed || ""}</div>{!it.licenseNote && <div className="text-amber-600">授權狀態未標記，建議只作為私人參考。</div>}</div>)}</div>}
  </div>;
}
