    ...
    <div className="space-y-4">
      ...
      <Card className="p-5 space-y-3">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {PROVIDERS.map((pv) => {
            const on = spine.provider === pv;
            return (
              <button
                key={pv}
                onClick={() => pick(pv)}
                className={`rounded-xl border p-3 text-left transition-colors ${on ? "border-primary bg-primary/5 ring-1 ring-primary" : "hover:bg-muted/40"}`}
              >
                <div className="flex items-center justify-between">
                  <span className="font-mono text-sm font-semibold uppercase">{pv}</span>
                  {on && <Badge variant="default" className="text-[10px]">使用中</Badge>}
                </div>
                <div className="text-[11px] text-muted-foreground mt-1.5">{PROVIDER_META[pv].note}</div>
                <div className="text-[11px] text-amber-600 font-mono mt-1.5">${PROVIDER_META[pv].cost}/張</div>
              </button>
            );
          })}
        </div>
